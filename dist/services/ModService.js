 ...
 */
class ModService {
    constructor(database, translationService, localModService) {
    ...
    async scanAndSyncLocalMods() {
        try {
            logger_1.logger.info('Starting local mod scan (offline mode)...');
            ...
            for (const modId of modIds) {
                try {
                    ...
                    if (mod.language && mod.language !== 'en') {
                        ...
                        if (needsTranslation) {
                            logger_1.logger.info(`Mod ${mod.id} needs translation`);
                    ...
                    synced.push(mod);
                    logger_1.logger.debug(`Processed mod: ${mod.title} (${mod.id})`);
                }
                catch (error) {
            ...
            logger_1.logger.info(`Scan complete: ${modIds.length} scanned, ${synced.length} synced, ${errors.length} errors`);
            return {
     ...
     */
    shouldTranslateMod(mod, existingMod) {
    ...
    async getMod(id, includeTranslation = true) {
        ...
        if (!mod) {
            logger_1.logger.debug(`Mod ${id} not found in database`);
            return null;
        }
        ...
        if (includeTranslation && mod.language && mod.language !== 'en' && !mod.translatedTitle) {
            try {
                ...
                logger_1.logger.error(`Failed to translate mod ${id}:`, error);
     ...
     */
    async exportMods(modIds, outputPath) {
        try {
            logger_1.logger.info(`Exporting ${modIds.length} mods to ${outputPath}`);
            ...
            if (missingMods.length > 0) {
                logger_1.logger.warn(`Missing local files for mods: ${missingMods.join(', ')}`);
            }
            ...
            return new Promise((resolve, reject) => {
                ...
                output.on('close', () => {
                    ...
                    logger_1.logger.info(`Export complete: ${modPaths.length} mods exported, ${sizeInMB} MB total`);
                    resolve({
                        zipPath: outputPath,
                ...
                for (const mod of modPaths) {
                    logger_1.logger.debug(`Adding mod ${mod.id} to archive`);
            ...
            logger_1.logger.error('Failed to export mods:', error);
            throw error;
        }
    }
}
